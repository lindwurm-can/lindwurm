/*  www.lindwurm-can.org
 *  Copyright (C) 2023 Sascha Muenzberg <sascha@lindwurm-can.org>
 *
 *  This program is free software: you can redistribute it and/or modify
 *  it under the terms of the GNU Affero General Public License as published by
 *  the Free Software Foundation, either version 3 of the License, or
 *  (at your option) any later version.
 *
 *  This program is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *  GNU Affero General Public License for more details.
 *
 *  You should have received a copy of the GNU Affero General Public License
 *  along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */

#ifndef GENERALSETTINGSWIDGET_H
#define GENERALSETTINGSWIDGET_H

#include <ioptionspage.h>

namespace Ui { class GeneralSettingsWidget; }

namespace Lindwurm::Core
{
    /**
     * @brief The GeneralSettingsWidget class provides the widget for the general Lindwurm settings.
     */
    class GeneralSettingsWidget : public IOptionsPageWidget
    {
        Q_OBJECT
        public:

            explicit        GeneralSettingsWidget();
                            ~GeneralSettingsWidget();

            virtual bool    apply() override;

        private:

            Ui::GeneralSettingsWidget   *ui;
            bool                        m_darkModeInitialSetting;
    };
}

#endif // GENERALSETTINGSWIDGET_H
